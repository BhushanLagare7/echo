...
export const ConversationIdView = ({
  ...
  const handleToggleStatus = async () => {
    ...
    if (conversations.status === "unresolved") {
      newStatus = "escalated";
    } else if (conversations.status === "escalated") {
      newStatus = "resolved";
    } else {
  ...
  if (conversations === undefined || messages.status === "LoadingFirstPage") {
      ...
      <AIConversation className="max-h-[calc(100vh-180px)]">
        <AIConversationContent>
          {Array.from({ length: 10 }).map((_, index) => {
            ...
            const widths = ["w-48", "w-60", "w-72", "w-96"];
            ...
            return (
              ...
              >
                <Skeleton
                  className={`h-9 rounded-lg ${width} bg-neutral-200`}
                />
